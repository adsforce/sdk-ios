...
@interface AdsforceIAPParameter ()
{
    AdsforceIAPModel *_model;
}
@end

@implementation AdsforceIAPParameter
...
- (void)createDataForAdvertiserWithAppStore {
    NSString *paramsStr = @"";
    if (_model.params) {
        paramsStr = [AdsforceUtil dictionaryToJson:_model.params];
        if (paramsStr != nil && ![paramsStr isEqualToString:@""]) {
            NSString *str = [AdsforceUtil URLEncodedString:paramsStr];
            paramsStr = str;
        }
    }
